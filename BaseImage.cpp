...
*/
BOOL CBaseImage::LoadBmp(LPCTSTR filename)
{
...
bmp 파일을 저장한다.
*/
BOOL CBaseImage::SaveImage(LPCTSTR file)
{
	DWORD size,len;
	if(file)
		_tcscpy(m_strFileName,file);
...
BOOL CBaseImage::LoadJpg(LPCTSTR file)
{
	m_pJpg=new CJpeg;
	m_pJpg->LoadJPG((LPTSTR)file);
	
	SIZE size;
	size.cx=m_pJpg->GetWidth();
	size.cy=m_pJpg->GetHeight();	
	Create(size, 0,RGB(0,0,0));	
	int Width = (size.cx * 3 + 3)/4*4;
	memcpy(m_pDib, m_pJpg->m_pData, Width * size.cy);
	delete m_pJpg;
	m_pJpg=NULL;
	return TRUE;
}

void CBaseImage::FlipY()
{
	int N = 3;
...
BOOL CBaseImage::SaveJpg(LPCTSTR file)
{
	m_pJpg=new CJpeg;
	FlipY();
	m_pJpg->SaveJPG((LPSTR)file,GetSize().cx,GetSize().cy,(BYTE *)m_pDib);
	FlipY();
	delete m_pJpg;
	m_pJpg=NULL;
	return TRUE;	
